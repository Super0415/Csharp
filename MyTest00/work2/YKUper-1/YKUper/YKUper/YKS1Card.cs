...
namespace Yungku.Common.IOCardS1
{
	public class YKS1Card
	{
        ...
        /// <param name="cmd"></param>
        /// <returns></returns>
        protected long GetPositionValue(string cmd)
        {
            try
            {
                string ret = ExecuteCommand(cmd);
                int high = int.Parse(ret.Substring(0, 5));
                int low = int.Parse(ret.Substring(5, 5));

                if (high >> 15 == 0x1) // 值为负数
                {
                    long Sum= high * 65536 + low;
                    return Sum;
                }
                else return (high*65536 + low);
            }
            catch
            {
                return ErrNum;
        ...
        {
            return ExecuteAndCheckOk(string.Format("SY {0:D3}", state)); 
        }
